#!/usr/bin/env python3
"""Validate AV4 backlog metadata table schema.
...
def validate(path: Path) -> list[ValidationError]:
    ...
    if len(rows) < 3:
        return [ValidationError(line=1, message="missing AV4 backlog metadata table")]
    ...
    for line_no, cells in rows[2:]:
        ...
        ticket_id, priority, effort, ticket, dod, test_plan, pr_split = cells

        if not ID_RE.match(ticket_id):
            errors.append(ValidationError(line=line_no, message=f"invalid ID '{ticket_id}' (expected AV4-###)"))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate AV4 backlog metadata table schema")
    parser.add_argument(
        "--file",
        default="docs/AUTONOMOUS_V4_BACKLOG.md",
        help="Path to AV4 backlog markdown file",
    )
    ...
    if errors:
        print("[FAIL] AV4 backlog metadata schema check failed")
        for err in errors:
            print(f"  - {path}:{err.line}: {err.message}")
        return 1

    print("[PASS] AV4 backlog metadata schema check passed")
    return 0
